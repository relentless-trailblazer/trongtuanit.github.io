...
function checkBalance(listAccounts) {
  return listAccounts.balance > 500000;
}

const listBalance = listAccounts.filter(checkBalance);


// loại id ra khỏi tài khoản, nếu chưa có thuộc tính active thì đặt isActive = null
// nếu đã có thì isActive = true
const newListAccount = listAccounts.map(function(username,balance,type,isActive){
  return {username:username, balance:balance, type:type, isActive:'true'}
});
